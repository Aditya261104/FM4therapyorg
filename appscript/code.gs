/**
 * FM4 CRM — downstream Meta CAPI event firing engine (webinar funnel).
 *
 * Bound to the FM4 CRM Google Sheet. On every USER edit to a trigger column
 * (attended / qualified / sale_closed) this script reads the lead's row,
 ...
 *
 * Funnel model: a single scheduled 2-day live workshop. Everyone who buys the
 * ₹97 ticket attends the SAME event, so `showup_time` (col Y) is identical for
 * all attendees. The high-ticket offer is sold manually by the sales team
 * after the workshop (not through the website funnel) — hence the qualified +
 * high-ticket lifecycle columns are filled by hand.
 *
 * Architecture: this script talks to Meta directly (no middleware proxy through
 * the Next.js backend). All secrets live in Apps Script PropertiesService. A
 * new client can be onboarded by copy-pasting this file into their Sheet's
 * Apps Script editor and setting three script properties.
 *
 ...
 * pixel + token but never talk to each other — the Sheet is the only link.
 *
 * Source-of-truth schema: 36-column row, A..AJ, as defined in sheet-header.csv.
 */
...
const COL = {
  ...
  ATTENDED:                  24,  // X   (dropdown TRUE/FALSE) → LeadShowUp
  SHOWUP_TIME:               25,  // Y   (workshop datetime; same for all attendees)
  LEADSHOWUP_CAPI_EVENT_ID:  26,  // Z
  LEADSHOWUP_CAPI_SENT:      27,  // AA
  QUALIFIED:                 28,  // AB  (dropdown TRUE/FALSE) → QualifiedLead
  QUALIFIED_TIME:            29,  // AC
  QUALIFIED_CAPI_EVENT_ID:   30,  // AD
  QUALIFIED_CAPI_SENT:       31,  // AE
  SALE_CLOSED:               32,  // AF  (dropdown TRUE/FALSE) → HighTicketPurchase
  CONTRACTED_VALUE:          33,  // AG  (plain integer, INR)
  SALES_TIME:                34,  // AH
...
function setupTriggers() {
  ...
  console.log(msg);
  SpreadsheetApp.getActive().toast(msg, 'FM4 CRM', 5);
}
...
function replayPendingEvents() {
  ...
  SpreadsheetApp.getActive().toast('Replayed ' + replayed + ' pending event(s)', 'FM4 CRM', 5);
}
